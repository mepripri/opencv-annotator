...
def detect_columns(img):
    ...
    for x in local_minima:
        ...
        if len(left_region) > 0 and len(right_region) > 0:
            ...
            current_val = smoothed[x]
            
            if current_val < 0.7 * left_max and current_val < 0.7 * right_max:
                filtered_separators.append(x)
    
    min_distance = w // 10
    column_separators = []
    
    if len(filtered_separators) > 0:
        column_separators.append(filtered_separators[0])
        for x in filtered_separators[1:]:
            if x - column_separators[-1] >= min_distance:
...
def detect_text_lines_in_column(column_img, min_text_density=0.02):
    ...
    h_kernel = np.ones(h_window) / h_window
    h_smoothed = np.convolve(horizontal_projection, h_kernel, mode='same')
    ...
    if text_regions[0]:
        line_starts = np.concatenate([[0], line_starts])
    
    if text_regions[-1]:
        line_ends = np.concatenate([line_ends, [h-1]])
    ...
    for start, end in zip(line_starts, line_ends[:len(line_starts)]):
        ...
        line_binary = cv2.threshold(
            cv2.cvtColor(line_region, cv2.COLOR_BGR2GRAY) if len(line_region.shape) == 3 else line_region,
            0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU
        )[1]
        
        text_pixels = np.sum(line_binary > 0)
        total_pixels = line_binary.size
...
def save_and_close(image_path, output_json_path='output.json'):
    boxes = auto_detect_boxes(image_path)
    if not boxes:
        print("Warning: No boxes were detected.")
    try:
...
if __name__ == "__main__":
    ...
    save_and_close(IMAGE_FILE, OUTPUT_FILE)